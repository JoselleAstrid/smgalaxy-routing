...
(function() {
  var Action, Event, Item, Level, Main, Message, MessageUtil, Route, determineArgSets,
  ...
  Level = (function(superClass) {
    ...
    Level.prototype.starName = function(langCode, character) {
      ...
      box = Message.idLookup[this.starNameId].computeBoxes(argSet, null)[0];
      return box.text;
    };
  ...
  Message = (function() {
    ...
    function Message(id1, data1) {
      this.id = id1;
      this.data = data1;
      this.constructor.idLookup[this.id] = this;
    }

    Message.prototype.computeBoxes = function(argSet, messageCase) {
      var box, boxes, content, item, j, k, lastBox, len, len1, newlineAfterBoxBreak;
      content = this.data[argSet.langCode];
      boxes = [
        {
          chars: 0,
          text: ""
    ...
    Message.prototype.showFrameDetails = function(argSet, messageCase) {
      var $backButton, $h3, $messageDetails, $table, $tbody, $td, $tr, box, boxes, data, j, len;
      data = this.data[argSet.langCode];
    ...
    Main.prototype.init = function(itemDetails, itemMessages, messages) {
      ...
      document.getElementById('route-button').onclick = (function(_this) {
        return function(event) {
          var argSets, category, route, routeText;
          $('#route-status').empty();
